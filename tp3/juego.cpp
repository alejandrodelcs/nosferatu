...
void Juego::atacar(Ser *personaje, Tablero *tablero) {
    int coordenadaX, coordenadaY;
    int danio;
    std::cout<<"Ingrese una coordenadaX: ";
    std::cin>>coordenadaX;
    std::cout << "Ingrese una coordenadaY: " << std::endl;
    std::cin>>coordenadaY;
    if (! tablero->obtenerCasillero(coordenadaX, coordenadaY)->hayPersonaje())
    	std::cout << "No hay personaje en esta posición." << std::endl;
    else{
		danio = personaje->atacar(coordenadaY,coordenadaX,tablero->obtenerCasillero(coordenadaX,coordenadaY)->obtenerPersonaje()->obtenerSimbolo());
		tablero->obtenerCasillero(coordenadaX,coordenadaY)->obtenerPersonaje()->recibir_ataque(danio);
		std::cout<<"123"<<std::endl;
		if(tablero->obtenerCasillero(coordenadaX,coordenadaY)->obtenerPersonaje()->obtenerVida() <= 0){
			if(jugador1->estaEnLaLista(tablero->obtenerCasillero(coordenadaX,coordenadaY)->obtenerPersonaje())) {
				jugador1->eliminarDato(jugador1->obtenerPosicion(tablero->obtenerCasillero(coordenadaX, coordenadaY)->obtenerPersonaje()));
				//delete tablero->obtenerCasillero(coordenadaX, coordenadaY)->obtenerPersonaje();
				//tablero->obtenerCasillero(coordenadaX, coordenadaY)->quitarPersonaje();
			}
			else {
				jugador2->eliminarDato(jugador2->obtenerPosicion(tablero->obtenerCasillero(coordenadaX, coordenadaY)->obtenerPersonaje()));
				//delete tablero->obtenerCasillero(coordenadaX, coordenadaY)->obtenerPersonaje();
				//tablero->obtenerCasillero(coordenadaX, coordenadaY)->quitarPersonaje();
			}
		}
