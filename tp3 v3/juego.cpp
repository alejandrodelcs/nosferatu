...
void Juego::atacar(Ser *personaje, Tablero *tablero) {
    int fila,col;
    int danio;
    std::cout<<"ingrese una fila y columna "<<std::endl;
    std::cin>>col;
    std::cin>>fila;
    danio=personaje->atacar(fila,col,tablero->obtenerCasillero(fila,col)->obtenerPersonaje()->obtenerSimbolo());
    tablero->obtenerCasillero(fila,col)->obtenerPersonaje()->recibir_ataque(danio);
    std::cout<<"123"<<std::endl;
    if(tablero->obtenerCasillero(fila,col)->obtenerPersonaje()->obtenerVida()<=0){
        if(jugador1->estaEnLaLista(tablero->obtenerCasillero(fila,col)->obtenerPersonaje())) {
            jugador1->eliminarDato(jugador1->obtenerPosicion(tablero->obtenerCasillero(fila, col)->obtenerPersonaje()));
            delete tablero->obtenerCasillero(fila, col)->obtenerPersonaje();
            tablero->obtenerCasillero(fila, col)->quitarPersonaje();
        }
        else {
            jugador2->eliminarDato(jugador2->obtenerPosicion(tablero->obtenerCasillero(fila, col)->obtenerPersonaje()));
            delete tablero->obtenerCasillero(fila, col)->obtenerPersonaje();
            tablero->obtenerCasillero(fila, col)->quitarPersonaje();
        }
    }
